...
class Renderer:
	...
	def _joined_source(self, joins, pushed_sql: str) -> str:
		...
		SurrealDB has no JOIN. Each join step is a sub-select per row of the previous step that looks up the matching rows of
		the next table (an index lookup when the ON criterion is an equality on an indexed shadow), flattened. A LEFT JOIN keeps
		a row without a match, with the joined side NONE (which every predicate treats as NULL)."""
		ctxs = self.ctxs
